...
describe('summary metrics', () => {
  it('excludes the first selected sample distance', () => {
    const metrics = calculateSummaryMetrics([
      sample('2026-08-15T10:00:00Z', { dist: 100 }),
      sample('2026-08-15T10:00:10Z', { dist: 20 }),
      sample('2026-08-15T10:00:20Z', { dist: 30 }),
    ])
  ...
  it('calculates Avg SOG from distance and elapsed time, not sample SOG', () => {
    const metrics = calculateSummaryMetrics([
      sample('2026-08-15T10:00:00Z', { dist: 500, sog: 40 }),
      sample('2026-08-15T10:30:00Z', { dist: 926, sog: 0 }),
      sample('2026-08-15T11:00:00Z', { dist: 926, sog: 20 }),
    ])
  ...
  it('returns unavailable Avg SOG without a positive elapsed interval', () => {
    ...
    expect(calculateSummaryMetrics([
      sample('2026-08-15T10:00:00Z'),
    ]).avgSogKnots).toBeNull()
    expect(calculateSummaryMetrics([
      sample('2026-08-15T10:00:00Z'),
      sample('2026-08-15T10:00:00Z', { dist: 10 }),
    ]).avgSogKnots).toBeNull()
  })

  it('groups 359° and 1° in the same north bin', () => {
    const metrics = calculateSummaryMetrics([
      sample('2026-08-15T10:00:00Z', { cog: 359 }),
      sample('2026-08-15T10:00:01Z', { cog: 1 }),
    ])
  ...
  it('selects the wrapped north bin for values from 356° through 4°', () => {
    const metrics = calculateSummaryMetrics([
      sample('2026-08-15T10:00:00Z', { cog: 356 }),
      sample('2026-08-15T10:00:01Z', { cog: 359 }),
      sample('2026-08-15T10:00:02Z', { cog: 1 }),
      sample('2026-08-15T10:00:03Z', { cog: 4 }),
      sample('2026-08-15T10:00:04Z', { cog: 92 }),
    ])
  ...
  it('bins ordinary and normalized COG values deterministically', () => {
    const ordinary = calculateSummaryMetrics([
      sample('2026-08-15T10:00:00Z', { cog: 86 }),
      sample('2026-08-15T10:00:01Z', { cog: 89 }),
      sample('2026-08-15T10:00:02Z', { cog: 94 }),
    ])
    const normalized = calculateSummaryMetrics([
      sample('2026-08-15T10:00:00Z', { cog: -1 }),
      sample('2026-08-15T10:00:01Z', { cog: 361 }),
    ])
    const tied = calculateSummaryMetrics([
      sample('2026-08-15T10:00:00Z', { cog: 11 }),
      sample('2026-08-15T10:00:01Z', { cog: 21 }),
    ])
  ...
  it('averages signed HEEL and TRIM while ignoring missing/non-finite values', () => {
    const metrics = calculateSummaryMetrics([
      sample('2026-08-15T10:00:00Z', { heel: -6, trim: 3 }),
      sample('2026-08-15T10:00:01Z', { heel: 2, trim: -1 }),
      sample('2026-08-15T10:00:02Z', { heel: null, trim: Number.NaN }),
    ])
